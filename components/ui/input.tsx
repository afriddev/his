...
const Input = React.forwardRef<HTMLInputElement, InputInterface>(({ className, type, label, errorMessage, mandatory, icon, onClear, ...props }, ref) => {
  function getIcon() {
    ...
    <div className={`flex flex-col gap-1 ${label && 'h-16'}`}>
      <div>
        ...
        <div className="relative flex items-center ">
          ...
          />
          {onClear && props?.value && (
            <div className="absolute  right-2 cursor-pointer w-5 h-5 bg-gray-100 lg:hover:bg-gray-300 flex items-center justify-center rounded-full" onClick={onClear}>
              <X className="h-4 w-4" />
            </div>
          )}
        </div>
      </div>
      {errorMessage && <p className="text-destructive">{<label className="">{errorMessage}</label>}</p>}
    </div>
  );
});
